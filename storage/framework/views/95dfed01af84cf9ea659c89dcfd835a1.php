                                ...
                                <?php $__currentLoopData = $musorok; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $musor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="list-group-item d-flex justify-content-between align-items-center">
                                        <div class="d-flex align-items-center">
                                            <input type="checkbox" name="musor_ids[]" value="<?php echo e($musor->id); ?>" class="form-check-input me-3 musor-checkbox">
                                            <div>
                                                <h5 class="mb-1"><?php echo e($musor->cim); ?></h5>
                                                <small>Feltöltve: <?php echo e(date('Y.m.d', strtotime($musor->created_at))); ?></small>
                                            </div>
                                        </div>
                                        <div>
                                            <button type="button" class="btn btn-sm btn-primary me-2" onclick="editMusor(<?php echo e($musor->id); ?>, '<?php echo e($musor->cim); ?>', '<?php echo e($musor->leiras); ?>', '<?php echo e($musor->kategoria); ?>', '<?php echo e($musor->kep_url); ?>')">
                                                <i class="bi bi-pencil-fill"></i>
                                            </button>
                                        </div>
                                    </div>
...
<script>
    ...
    nameBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('<?php echo e(route('nev.valtoztatas')); ?>', {
                method: 'POST',
                headers: {
    ...
    function deleteSelected() {
        ...
        fetch('<?php echo e(route("musor.torles")); ?>', {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                'Content-Type': 'application/json',
    ...
    function editMusor(id, cim, leiras, kategoria, kepUrl) {
        document.getElementById('edit_id').value = id;
        document.getElementById('edit_cim').value = cim;
        document.getElementById('edit_leiras').value = leiras;
        document.getElementById('edit_kategoria').value = kategoria;
        document.getElementById('current_image').src = '/uploads/teliprojekt/' + kepUrl;
    ...
    document.getElementById('editForm').addEventListener('submit', function(e) {
        ...
        fetch('<?php echo e(route("musor.szerkesztes")); ?>', {
            method: 'POST',
            body: formData,
            headers: {
                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php echo $__env->make('teliprojekt.layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/vhosts/moriczcloud.hu/egyedirobi.moriczcloud.hu/resources/views/teliprojekt/profil.blade.php ENDPATH**/ ?>